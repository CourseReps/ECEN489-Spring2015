...
/* Make sure to include the following libraries in your project directory:
 * -Google API\libs
 * -google-api-services-fusiontables-v2-rev1-1.19.jar
 * -SERVLET API TOMCAT ---.jar
...
public class FusionTablesSample {
   ...
   */
  private static final String APPLICATION_NAME = "Sample Project"; //The name specified here is project name specified in Google Developers Console
   ...
   */
  /*Global Instances*/
  private static FileDataStoreFactory dataStoreFactory;                                 //Global instance of the
  private static HttpTransport httpTransport;                                           //Global instance of HTTP transport
  ...
  private static String TableName = "DATA";             //Specifies the Name of the Fusion and Database Table(Should be same)//If not, we can add another string.
  private static String column1 = "TIME";                 //Specifies the Column Names for both the tables
  private static String column2 = "NUM_PEOPLE";
  private static String column3 = "PBID";                 //Promiscuous Box ID Number
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Connection conn = DriverManager.getConnection("jdbc:sqlite:G:/Project1Server/mainAnalysis.db"); //SQL Database file path//jdbc:sqlite:C:/Users/Evelyn/Documents/sqlite/mainAnalysis.db"
      Statement stat = conn.createStatement();
      ResultSet rs = stat.executeQuery("select * from "+TableName+";");                                       //Calls Data from specified table
      while (rs.next()) {
        insertData(tableId,rs.getString(column1),rs.getString(column2),rs.getString(column3));      //Data being called one by one from table
  ...
  /** Create a table for the authenticated user if the table does not already exist. */
