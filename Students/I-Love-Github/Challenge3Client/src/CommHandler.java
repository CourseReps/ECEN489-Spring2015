...
class CommHandler implements Runnable {
    ...
    private void collectAndSendData() {
        ...
        try {
            ResultSet xmitList = parent.getDB().gatherDataForServer();

            while (xmitList.next()) {
                key = String.valueOf(xmitList.getInt("ID"));
                time = String.valueOf(xmitList.getLong("TIME"));
                name = xmitList.getString("NAME");
                data = xmitList.getString("DATA");
