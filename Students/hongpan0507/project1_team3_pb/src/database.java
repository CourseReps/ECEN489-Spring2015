...
public class database {
    ...
    public database (String _db_name) {
        db_name = "jdbc:sqlite:/home/ecen489-spring/workspace_sqlite/" + _db_name;
    }
    ...
    public void insert_val_string(String _table_name, String[] data) throws Exception {
        sql = "INSERT INTO " + _table_name + " (";
        for (int i = 0; i < data.length; ++i) {
            sql = sql + column_name[i];
            if(i != data.length -1) sql = sql + ",";
        }
        sql = sql + ")VALUES (";
        for (int i = 0; i < data.length; ++i) {
            sql = sql + "\'" + data[i] + "\'";
    ...
    public void insert_val_number(String _table_name, String[] data) throws Exception {
        sql = "INSERT INTO " + _table_name + " (";
        for (int i = 0; i < data.length; ++i) {
            sql = sql + column_name[i];
            if(i != data.length -1) sql = sql + ",";
    ...
    public void single_stmt(String stmt) throws Exception{
        db_stmt.executeUpdate(stmt);
        db_con.commit();
    }
